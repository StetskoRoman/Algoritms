...
public class Task3Dirty {
    ...
    static boolean solution(List<Pair> pairList) {
        ...
        System.out.println(addedGift);

//        int currentDay = pairList.get(0).readyDay;
        int currentDay = 0;
        int readyNow = 0;
        int alreadSent = 0;
        int currentI = 0;
...
//            }
            ...
            System.out.println("added list = " + addedGift  +  " readyNow = " + readyNow + "  readyList = "  + readyList.isEmpty() + "   alreadySent = " + alreadSent + "  curr day = " + currentDay);
            for (int i = 0; i < readyList.size(); i++) {
                if (currentDay > readyList.get(i).lastDay) {
                    System.out.println("return 111 false");
                    return false;
                }
            }
            if (!readyList.isEmpty()) {
                ...
                System.out.println(readyList + "  ready now " + readyNow + "     already sent " + alreadSent + "     curr day " + currentDay);
//                if (alreadSent == n && readyNow == 0) {
...
//                }
    ...
    static int[] getGiftForSending(List<Pair> readyList, int readyNow, int alreadySent, int currentDay) {
        ...
        System.out.println("before readyList " + readyList);
        ...
        System.out.println("after readyList " + readyList + Arrays.toString(resultCondition));
    ...
    static class Pair implements Comparable<Pair> {
        ...
        public boolean equals(Object o) {
            ...
            if (!(o instanceof Task4.Pair pair)) return false;
            return readyDay == pair.interest && lastDay == pair.feed;
        }
