...
//        int[][] prefixMatrix = getPrefixMatrix(list);
        int[][] absStolb = absSumStolb(list);
        int[][] absArray = absSumArray(list);
        int[][] prefixMatrix = getPrefixMatrix(list, absStolb, absArray);
    ...
    static int[][] getPrefixMatrix(List<int[]> list, int[][] absStolb, int[][] absArray) {
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 1; j < m; j++) {
//                bad approach, lazy
                prefixMatrix[i][j] = prefixMatrix[i - 1][j - 1] + absStolb[i][j] + absArray[i][j] - list.get(i)[j];
//                prefixMatrix[i][j] = prefixMatrix[i - 1][j - 1] + sumStolb(list, i, j) + sumArray(list, i, j) - list.get(i)[j];
    ...
    static int[][] absSumStolb(List<int[]> list) {
        int[][] absStolb = new int[list.size()][list.get(0).length];

        for (int j = 0; j < list.get(0).length; j++) {
            for (int i = 1; i < list.size(); i++) {
                absStolb[i][j] = absStolb[i - 1][j] + list.get(i)[j];
            }
        }
    ...
    static int[][] absSumArray(List<int[]> list) {
        int[][] absArray = new int[list.size()][list.get(0).length];

        for (int i = 0; i < list.size(); i++) {
            for (int j = 1; j < list.get(0).length; j++) {
                absArray[i][j] = absArray[i][j - 1] + list.get(i)[j];
            }
        }
        return absArray;
    }
